...
2. **Студент**: Подключается к лекции через Telegram-бота по QR-коду или названию. Получает слайды автоматически, задаёт вопросы текстом.
...
| **FR-UR-02** | User Requirement | Лектор должен получать изображение текущего слайда, автоматически отправляемое студентам при каждой смене слайда. |
...
| **FR-UR-09** | User Requirement | Студент должен автоматически получать изображения слайдов при каждой смене без каких-либо дополнительных действий. |
| **FR-UR-10** | User Requirement | Студент должен иметь возможность запросить текущий слайд повторно командой `/slide`. |
...
| **FR-SR-02** | System Requirement | При получении события смены слайда сервер обязан захватить SVG-представление слайда, конвертировать в PNG и разослать изображение всем активным подписчикам через Telegram Bot API. |
...
- **Конфиденциальность**: Рассылка слайдов и уведомлений осуществляется только студентам с активным статусом подключения к текущей сессии лекции.
...
6. Надстройка отправляет `POST /lecture/{id}/slides` с изображением
7. Надстройка отправляет `POST /lecture/{id}/slide-changed` с номером слайда
8. Сервер вызывает `TelegramNotificationAdapter` → рассылает фото всем студентам
9. Каждый студент получает изображение слайда в Telegram
