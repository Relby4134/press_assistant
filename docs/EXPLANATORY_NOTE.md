...
**Выигрыш от ПП** — мгновенное уведомление студентов о смене слайда с возможностью запросить его изображение через Telegram-бот; возможность задавать вопросы анонимно прямо в чате без прерывания лекции; аналитика активности студентов и история вопросов для преподавателя.
...
Учебные заведения (университеты, колледжи, школы), проводящие лекционные занятия с использованием презентаций Microsoft PowerPoint. Система применима как для очного формата (студенты в аудитории), так и для смешанного (часть студентов присутствует дистанционно).

**От каких систем/пользователей поступают входные данные ПС?**
- **Лектор** — управляет лекцией через надстройку PowerPoint: запускает сессию, переключает слайды, завершает лекцию
- **Студент** — подключается к лекции и запрашивает слайды через Telegram-бот
- **PowerPoint** — генерирует события смены слайда и предоставляет SVG-изображение текущего слайда

**Кто/что использует результаты (потребители, включая другие системы и сервисы)?**
- **Студенты** — получают уведомления о текущем слайде и изображения слайдов по запросу в Telegram
- **Лектор** — видит список вопросов, состав аудитории, аналитику запросов слайдов
- **Telegram** — выступает каналом доставки уведомлений и изображений студентам

**Какие сторонние системы/сервисы участвуют в процессе обработки данных?**
- **Telegram Bot API** — канал уведомлений, доставки изображений и приёма вопросов
- **Microsoft PowerPoint / Office Add-in API** — среда работы лектора, источник событий смены слайда
- **Windows Certificate Store** — хранилище доверенных SSL-сертификатов, необходимых для HTTPS-соединения

---

### 3. Контекстная диаграмма

![](./images/image_context.png)

```
┌────────────────┐        HTTPS/REST       ┌─────────────────────────┐
│   PowerPoint   │◄───────────────────────►│   PresAssistant Server  │
│  (Add-in JS)   │   slide-changed, /start │   Spring Boot + H2      │
└────────────────┘                         └───────────┬─────────────┘
       ▲                                               │
       │ Office.js API                     Telegram Bot API (HTTPS)
  Лектор                                               │
                                           ┌───────────▼─────────────┐
                                           │    Telegram Cloud       │
                                           └───────────┬─────────────┘
...
#### Таблица A. Сравнительный анализ конкурирующих решений
...
| Работа без корпоративных аккаунтов и VPN | U, XS | + | — | — | e |
...
### 5. Образ программного продукта (шаблон Moore, 1991)
...
**«который»** — автоматически уведомляет студентов о смене слайда и позволяет им запросить его изображение через Telegram-бот, а также собирает вопросы аудитории без регистрации и установки отдельных приложений;
...
**«наш продукт»** — работает полностью локально, устанавливается одним MSI-файлом, не требует от студентов ничего, кроме Telegram, и не передаёт данные лекций сторонним облачным сервисам.
...
рисунок![](./images/image_usecase.png)
...
рисунок![](./images/image_architecture.png)
...
рисунок![](./images/image_launcher.png)
...
рисунок![](./images/image_addin_setup.png)
...
рисунок![](./images/image_addin_active.png)
...
рисунок![](./images/image_seq_start.png)
...
рисунок![](./images/image_seq_slide.png)
...
рисунок![](./images/image_classes.png)
